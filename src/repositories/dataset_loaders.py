...
from src.models.core import (
    Dataset, Conversation, ConversationTurn, DatasetType, Speaker
)
        ...
        try:
            with open(labels_file, 'r', encoding='utf-8') as f:
                ...
                if isinstance(labels_data, list):
                    ...
                    raise ValueError(f"Unexpected format for intent labels: {type(labels_data)}")
...
class BitextLoader(DatasetLoaderInterface):
    ...
    def _load_parquet(self, file_path: Path) -> List[Conversation]:
        ...
        try:
            ...
            logger.error("pandas is required to load Parquet files. Install with: pip install pandas pyarrow")
            raise
        except Exception as e:
...
class SchemaGuidedLoader(DatasetLoaderInterface):
    ...
    def load(self, path: Path) -> Dataset:
        ...
        logger.info(f"Loaded {len(conversations)} conversations from Schema-Guided dataset")
        ...
        try:
            ...
            for dialogue in dialogues:
                ...
                for turn_data in turns_data:
                    ...
                    speaker = Speaker.USER if speaker_str == 'USER' else Speaker.ASSISTANT
...
class TwitterSupportLoader(DatasetLoaderInterface):
    ...
    def load(self, path: Path) -> Dataset:
        ...
        logger.info(f"Loaded {len(conversations)} conversations from Twitter Support dataset")
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for idx, (conv_id, tweets) in enumerate(conversation_map.items()):
                    ...
                    for tweet in tweets:
                        ...
                        is_support = 'support' in tweet['author_id'].lower() or not tweet['in_response_to']
...
class SyntheticSupportLoader(DatasetLoaderInterface):
    ...
    def load(self, path: Path) -> Dataset:
        ...
        logger.info(f"Loaded {len(conversations)} conversations from Synthetic Support dataset")
        ...
        return Dataset(
            ...
            conversations=conversations,
            metadata={
                "source": "Synthetic Tech Support Chats",
                "file": str(file_path)
            }
        )
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for idx, row in enumerate(reader):
                    ...
                    text = row.get('text', '') or row.get('message', '') or row.get('utterance', '')
                    speaker_str = row.get('speaker', '') or row.get('role', 'user')
                    
                    speaker = Speaker.ASSISTANT if 'agent' in speaker_str.lower() or 'assistant' in speaker_str.lower() else Speaker.USER
